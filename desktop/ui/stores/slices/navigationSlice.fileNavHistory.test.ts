...
describe("recordFileVisit", () => {
  it("appends visited files and advances the cursor", () => {
    ...
    const { fileNavHistory, fileNavIndex } = useSpurStore.getState();
  ...
  it("drops forward history when visiting a new file after going back", () => {
    useSpurStore.getState().recordFileVisit("a.ts");
    useSpurStore.getState().recordFileVisit("b.ts");
    useSpurStore.getState().recordFileVisit("c.ts");
    useSpurStore.setState({ fileNavIndex: 0 }); // simulate having stepped back to a.ts
    useSpurStore.getState().recordFileVisit("d.ts");
    const { fileNavHistory, fileNavIndex } = useSpurStore.getState();
  ...
  it("caps the stack at 50 entries", () => {
    for (let i = 0; i < 60; i++) {
      useSpurStore.getState().recordFileVisit(`file${i}.ts`);
    }
    const { fileNavHistory, fileNavIndex } = useSpurStore.getState();
...
describe("navigateFileHistory", () => {
  ...
  it("steps back and selects the previous file", () => {
    useSpurStore.getState().navigateFileHistory(-1);
  ...
  it("is a no-op past the start of history", () => {
    useSpurStore.getState().navigateFileHistory(-1);
    useSpurStore.getState().navigateFileHistory(-1);
    useSpurStore.getState().navigateFileHistory(-1); // already at index 0
    const state = useSpurStore.getState();
  ...
  it("is a no-op past the end of history", () => {
    useSpurStore.getState().navigateFileHistory(-1); // b.ts, so forward has somewhere to go
    useSpurStore.getState().navigateFileHistory(1); // c.ts, back at the most recent entry
    useSpurStore.getState().navigateFileHistory(1); // already at the most recent entry
    const state = useSpurStore.getState();
  ...
  it("does not re-record the target file it navigates to", () => {
    ...
    useSpurStore
      .getState()
      .recordFileVisit(useSpurStore.getState().selectedFile!);
    const { fileNavHistory, fileNavIndex } = useSpurStore.getState();
