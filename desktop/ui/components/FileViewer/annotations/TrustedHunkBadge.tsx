...
import { hunkLabels, findMatchingPattern } from "../../../types";
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuContent,
...
export function TrustedHunkBadge({
  ...
  const trustedLabels = labels
    .map((lbl) => ({ label: lbl, pattern: findMatchingPattern(lbl, trustList) }))
    .filter(
      (entry): entry is { label: string; pattern: string } =>
