...
Single combined store in `stores/index.ts` via `useSpurStore` hook. State is split into 19 slices in `stores/slices/` — see `stores/types.ts` for the authoritative list. The ones you will touch most:
...
The app is a queue of workspaces and a stage showing one of them. A workspace is a container that becomes what you put in it: a nullable `title`, a `displayTitle` the backend derives when there is none, and an ordered list of **attachments** (`{path, refName}`) which are the code half's repo tabs. Nothing about an attachment is exclusive — any number of workspaces may show the same repo — and `refName` is a view hint, never identity: the tab is the _path_. A path is a **checkout**, so a linked worktree is a tab of its own beside the main tree's; the repository it belongs to rides along as `repoRoot`, and that is what every per-repo join reads (the review key, the sidebar row, the PR, the merge, `repoMetadata`). `ActiveReviewKey` carries both for the same reason: `repoPath` is the review's repository, `path` the checkout it is being read in, resolved once by `setActiveReviewKey` (`withTabPath`) since almost every route names a repo and a ref and no tab.
...
(`types/viewpoint.ts`): `spur` means the two are the same; `range` narrows to
...
Frontend logs are written to `~/.spur/app.log` (app-wide, not per-repo). All `console.log`, `console.warn`, `console.error`, `console.info`, and `console.debug` calls are captured with timestamps and log levels:
...
In dev mode, React Scan render events are written to `~/.spur/react-scan.jsonl` as JSONL (app-wide, not per-repo). Each line records a component render with timing, phase, and what changed. The log is cleared on app start. Logging is tied to React Scan's toolbar — pausing scanning pauses logging.
