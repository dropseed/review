...
**Skills**: one bundled skill, `spur-app`, covering all three surfaces an agent touches — reviewing a diff (hunks, trust, guide, comments), driving the app's terminals, and reading/feeding the workspace queue. The canonical source is `core/resources/skills/spur-app/SKILL.md`, `include_str!`-embedded into the binary so the shipped CLI carries it. `spur skill install` writes it into `~/.claude/skills/` and `$CODEX_HOME/skills/` (defaulting to `~/.codex/skills/`), and deletes the superseded `review-app` / `review-guide` / `review-terminals` skills it previously installed.

**An installed skill is kept current, not left behind.** The writing lives in `core/src/skill.rs` (ungated — the CLI and the app both call it), and the app refreshes every installed copy on launch. A stale skill is worse than a missing one: without it an agent reads `spur --help`, whereas a stale one confidently names flags that moved, and `SUPERSEDED` is that same argument one version earlier. But the file sits in the human's directory, so a refresh never guesses — beside the skill is `.spur-managed`, holding the digest of the content last written, and the comparison against what is on disk is the whole design. Digests match: ours, updated silently. Digests differ: they edited it, left alone and logged. **No marker at all: a launch does nothing** — an older CLI wrote that copy and a launch has no business claiming a file it can't prove it owns, nor creating a skill nobody asked for. `spur skill install` is the explicit half that adopts such a copy, and `--force` is the one thing that overwrites an edited one.
