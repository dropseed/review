...
**Skill**: `review skill install` writes the bundled `review-guide` skill into `~/.claude/skills/`. The skill's canonical source is `core/resources/skills/review-guide/SKILL.md`, `include_str!`-embedded into the binary so the shipped CLI carries it.
