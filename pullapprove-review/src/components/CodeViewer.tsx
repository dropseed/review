...
import type { SupportedLanguages, DiffLineAnnotation } from "@pierre/diffs/react";
...
function getLanguageFromFilename(filePath: string): SupportedLanguages | undefined {
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden animate-fade-in">
      ...
      <div className="flex items-center justify-between border-b border-stone-800/50 bg-stone-900 px-3 py-1.5">
        ...
        {!isUntracked && hasChanges && (
          <div className="flex items-center rounded bg-stone-800/30 p-0.5">
            <button
              onClick={() => { setViewMode("unified"); setHighlightLine(null); }}
            ...
            <button
              onClick={() => { setViewMode("split"); setHighlightLine(null); }}
              className={`rounded px-2 py-0.5 text-[10px] font-medium transition-all ${
                viewMode === "split"
                  ? "bg-stone-700/50 text-stone-200"
                  : "text-stone-500 hover:text-stone-300"
            ...
            <button
              onClick={() => { setViewMode("file"); setHighlightLine(null); }}
              className={`rounded px-2 py-0.5 text-[10px] font-medium transition-all ${
                viewMode === "file"
                  ? "bg-stone-700/50 text-stone-200"
                  : "text-stone-500 hover:text-stone-300"
      ...
      <div className="flex-1 overflow-auto scrollbar-thin bg-stone-950">
        {isUntracked ? (
          <UntrackedFileView content={fileContent.content} filePath={filePath} hunks={fileContent.hunks} />
        ) : hasChanges && viewMode !== "file" ? (
          <DiffView
            diffPatch={fileContent.diffPatch}
            viewMode={viewMode as "unified" | "split"}
            hunks={fileContent.hunks}
            onViewInFile={(line) => { setViewMode("file"); setHighlightLine(line); }}
          />
        ) : (
          <PlainCodeView content={fileContent.content} filePath={filePath} highlightLine={highlightLine} />
        )}
      </div>
    </div>
  );
...
function PlainCodeView({ content, filePath, highlightLine }: { content: string; filePath: string; highlightLine?: number | null }) {
  ...
  return (
    <div ref={containerRef}>
      <PierreFile
        ...
        selectedLines={highlightLine ? { start: highlightLine, end: highlightLine, side: "additions" } : null}
        options={{
          theme: {
            dark: "github-dark",
            light: "github-light",
...
function UntrackedFileView({ content, filePath, hunks }: UntrackedFileViewProps) {
  const { reviewState, approveHunk, unapproveHunk, rejectHunk, unrejectHunk, setHunkNotes } = useReviewStore();
  ...
  return (
    <div>
      {/* Approval controls */}
      {hunk && (
        <div className={`sticky top-0 z-10 mb-2 flex items-center gap-3 border-b border-stone-800/50 backdrop-blur-sm p-3 ${
          isRejected
            ? "bg-rose-500/10"
            : isApproved
          ...
          {isApproved ? (
            ...
            >
              <svg className="h-3.5 w-3.5 group-hover:hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
              </svg>
              <svg className="hidden h-3.5 w-3.5 group-hover:block" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
              <span className="text-[10px] opacity-60">{hunkState?.approvedVia}</span>
            </button>
          ) : !isRejected ? (
            <button
          ...
          <div className="relative">
            ...
            >
              <svg className="h-3.5 w-3.5" fill={hasNotes ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
interface HunkAnnotationMeta {
  hunk: DiffHunk;
  hunkState: { label?: string[]; approvedVia?: "manual" | "trust" | "ai"; rejected?: boolean; notes?: string } | undefined;
...
function DiffView({ diffPatch, viewMode, hunks, onViewInFile }: DiffViewProps) {
  const { reviewState, approveHunk, unapproveHunk, rejectHunk, unrejectHunk, setHunkNotes, hunks: allHunks, setSelectedFile } = useReviewStore();
  const [notesPopoverHunkId, setNotesPopoverHunkId] = useState<string | null>(null);
  ...
  const lineAnnotations: DiffLineAnnotation<HunkAnnotationMeta>[] = hunks.map((hunk) => {
    const hunkState = reviewState?.hunks[hunk.id];
    const pairedHunk = hunk.movePairId ? allHunks.find((h) => h.id === hunk.movePairId) ?? null : null;
    ...
    const firstChangedLine = hunk.lines.find((l) => l.type === "added" || l.type === "removed");
  ...
  const renderAnnotation = (annotation: DiffLineAnnotation<HunkAnnotationMeta>) => {
    ...
    return (
      <div className={`flex items-center gap-2 px-3 py-1.5 border-b border-stone-700/50 ${
        isRejected
          ? "bg-rose-500/10"
          : isApproved
          ? "bg-lime-500/5"
        ...
        {hunkState?.label?.map((lbl, i) => (
          <span key={i} className="rounded-full bg-amber-500/15 px-2 py-0.5 text-xs font-medium text-amber-400">
            {lbl}
          </span>
        ))}
        ...
        {pairedHunk && (
          ...
          >
            ...
            <span className="opacity-60">{pairedHunk.filePath.split("/").pop()}</span>
          </button>
        )}
        ...
        <div className="relative">
          <button
            onClick={() => setNotesPopoverHunkId(showNotesPopover ? null : hunk.id)}
            className={`rounded p-1.5 transition-colors ${
              hasNotes
                ? "text-amber-400 hover:bg-amber-500/20"
                : "text-stone-500 hover:bg-stone-700 hover:text-stone-300"
            ...
            {onViewInFile && (
              <button
                onClick={() => {
                  ...
                  const firstChanged = hunk.lines.find((l) => l.type === "added" || l.type === "removed");
                  const targetLine = firstChanged?.newLineNumber ?? hunk.newStart;
