  ...
  const collectChanged = (entries: FileEntry[]) => {
    for (const entry of entries) {
      if (entry.status && !entry.isDirectory &&
        ...
        const reviewedHunks = fileHunks.filter(h =>
          reviewState?.hunks[h.id]?.approvedVia
        ).length;

        // Count rejected hunks
        const rejectedHunks = fileHunks.filter(h =>
          reviewState?.hunks[h.id]?.rejected
        ).length;
        ...
        changedFiles.push({
          path: entry.path,
          name: entry.name,
          status: entry.status as "added" | "modified" | "deleted" | "renamed" | "untracked",
          reviewedHunks,
          rejectedHunks,
          totalHunks,
...
function FileRow({
  ...
  return (
    ...
    >
      ...
      {/* File name */}
      <span className={`flex-1 truncate font-mono text-xs ${isSelected ? "text-stone-100" : "text-stone-300"}`}>
        {file.path}
      </span>
      ...
      {file.totalHunks > 0 && (
        <span className="font-mono text-xs text-stone-500">
          ...
          {file.rejectedHunks > 0 && (
            <span className="text-rose-400 ml-1">({file.rejectedHunks} rejected)</span>
          )}
        </span>
      )}
      ...
      {file.totalHunks > 0 && (
        <button
          ...
          className={`opacity-0 group-hover:opacity-100 transition-opacity p-0.5 rounded hover:bg-stone-700 ${
            file.isComplete ? "text-stone-400 hover:text-rose-400" : "text-stone-400 hover:text-lime-400"
          }`}
      ...
      {/* Complete indicator */}
      {file.isComplete && (
        <svg className="h-3.5 w-3.5 text-lime-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
        </svg>
      )}
    </button>
...
export function ReviewFilePanel({
  ...
  const handleContextMenu = (e: React.MouseEvent, file: FileReviewStatus) => {
    ...
    const revealLabel = platformName === "macos" ? "Reveal in Finder" : platformName === "windows" ? "Reveal in Explorer" : "Reveal in Files";
  ...
  if (totalFiles === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-6 text-center">
        <svg className="mb-3 h-8 w-8 text-stone-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
  ...
  return (
    <>
    {contextMenu && (
      <ContextMenu menu={contextMenu} onClose={() => setContextMenu(null)} onRevealInTree={onRevealInTree} />
    )}
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin py-1">
        ...
        {pending.length === 0 && reviewed.length > 0 && (
          <div className="flex flex-col items-center py-6 text-center">
            <svg className="mb-2 h-6 w-6 text-lime-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
            </svg>
