...
import type { Comparison, FileEntry, DiffHunk, ReviewState, ClassifyResponse, MovePair, RejectionFeedback } from "../types";
import { makeComparison } from "../types";
import { anyLabelMatchesAnyPattern, anyLabelMatchesPattern } from "../utils/matching";
...
const createDebouncedSave = () => {
  ...
  return (saveFn: () => Promise<void>) => {
    ...
    saveTimeout = setTimeout(() => {
      saveFn().catch((err) => console.error("Failed to save review state:", err));
    }, 500);
  };
};
...
export const useReviewStore = create<ReviewStore>((set, get) => ({
  ...
  prevFile: () => {
    ...
    if (!selectedFile) {
      set({ selectedFile: flatFiles[flatFiles.length - 1], focusedHunkIndex: 0 });
      return;
    }

    const currentIndex = flatFiles.indexOf(selectedFile);
    const prevIndex = currentIndex <= 0 ? flatFiles.length - 1 : currentIndex - 1;
  ...
  loadFiles: async () => {
    ...
    try {
      ...
      const collectChangedPaths = (entries: FileEntry[]) => {
        for (const entry of entries) {
          if (entry.status && !entry.isDirectory &&
              ["added", "modified", "deleted", "renamed", "untracked"].includes(entry.status)) {
      ...
      for (const filePath of changedPaths) {
        try {
          const content = await invoke<{ hunks: DiffHunk[] }>("get_file_content", {
            repoPath,
            filePath,
            comparison,
          });
      ...
      try {
        const result = await invoke<DetectMovePairsResponse>("detect_hunks_move_pairs", {
          hunks: allHunks,
        });
  ...
  loadCurrentComparison: async () => {
    ...
    try {
      const savedComparison = await invoke<Comparison | null>("get_current_comparison", {
        repoPath,
      });
      if (savedComparison) {
        ...
        try {
          const defaultBranch = await invoke<string>("get_default_branch", { repoPath });
  ...
  setHunkLabel: (hunkId, label) => {
    ...
    const shouldAutoApprove =
      !existingHunk.approvedVia && anyLabelMatchesAnyPattern(labels, reviewState.trustList);
  ...
  addTrustPattern: (pattern) => {
    ...
    for (const [hunkId, hunkState] of Object.entries(newHunks)) {
      ...
      if (labels.length > 0 && anyLabelMatchesPattern(labels, pattern) && !hunkState.approvedVia) {
        newHunks[hunkId] = {
          ...hunkState,
          approvedVia: "trust",
        };
  ...
  removeTrustPattern: (pattern) => {
    ...
    for (const [hunkId, hunkState] of Object.entries(newHunks)) {
      const labels = hunkState.label || [];
      if (labels.length > 0 && anyLabelMatchesPattern(labels, pattern) && hunkState.approvedVia === "trust") {
        newHunks[hunkId] = {
          ...hunkState,
          approvedVia: undefined,
        };
  ...
  classifyUnlabeledHunks: async () => {
    ...
    try {
      ...
      const response = await invoke<ClassifyResponse>("classify_hunks_with_claude", {
        repoPath,
        hunks: hunkInputs,
      });
      ...
      const newHunks = { ...reviewState.hunks };
      for (const [hunkId, classification] of Object.entries(response.classifications)) {
