...
fn is_deletions_only(hunk: &DiffHunk) -> bool {
    hunk.lines.iter().all(|line| matches!(line.line_type, LineType::Removed | LineType::Context))
        && hunk.lines.iter().any(|line| matches!(line.line_type, LineType::Removed))
}

/// Check if a hunk consists only of added lines (additions-only)
fn is_additions_only(hunk: &DiffHunk) -> bool {
    hunk.lines.iter().all(|line| matches!(line.line_type, LineType::Added | LineType::Context))
        && hunk.lines.iter().any(|line| matches!(line.line_type, LineType::Added))
}
