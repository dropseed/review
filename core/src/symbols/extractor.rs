...
/// Recursively walks the symbol tree to find matches at any depth.
pub fn find_definitions(source: &str, file_path: &str, symbol_name: &str) -> Vec<SymbolDefinition> {
    let symbols = match extract_symbols(source, file_path) {
        Some(s) => s,
        None => return Vec::new(),
    };

    let mut results = Vec::new();
    collect_matching_definitions(&symbols, file_path, symbol_name, &mut results);
    results
}

/// Recursively collect symbols whose name matches the target.
fn collect_matching_definitions(
    ...
    for symbol in symbols {
        ...
        collect_matching_definitions(&symbol.children, file_path, target_name, results);
    }
}
        ...
        "type_declaration" => {
            ...
            for child in node.children(&mut cursor) {
                if child.kind() == "type_spec" {
                    ...
                    // struct/interface type itself. A grouped `type ( … )`
                    // declaration runs on to its `)`, past the spec this symbol
                    // describes, so a body range taken from the spec would span
                    // its siblings — claim one only when the two agree.
                    let body_start_line = (child.end_position().row as u32 + 1 == end_line)
                        .then(|| body_interior_start(type_node, source))
                        .flatten();
                    return Some(Symbol {
                        name,
                        kind,
...
"#;
        ...
        // Whatever body range a spec claims must stay inside its own symbol.
        for sym in &symbols {
            if let Some(body) = sym.body_start_line {
                assert!(body > sym.start_line && body <= sym.end_line, "{sym:?}");
            }
        }
    }

    /// A grouped `type ( … )` ends at its `)`, so a body range taken from the
    /// first spec would span the siblings after it — none is claimed instead.
    #[cfg(feature = "symbols-go")]
    #[test]
    fn test_grouped_go_type_declaration_claims_no_body() {
        let source = r#"
package main
...
"#;
        ...
        assert_eq!(foo.body_start_line, None);
    }
