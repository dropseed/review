...
function getSelectedHunksInfo(
	...
	const hunks = file.hunks.filter(
		(h) => h.startLine <= endLine && h.endLine >= startLine,
	);
...
export async function activate(
	...
	// Register tree view
	const treeView = vscode.window.createTreeView("pullapprove-review.files", {
	...
	// Watch for changes to .pullapprove/current file (for CLI sync)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("pullapprove-review.refresh", async () => {
			fileTreeProvider.refresh();
			await reviewViewProvider.refreshNotes();
			await decorationProvider.refresh();
		}),
		...
		vscode.commands.registerCommand(
			"pullapprove-review.toggleEditorDecorations",
			async () => {
				const config = vscode.workspace.getConfiguration("pullapprove-review");
		...
		vscode.commands.registerCommand(
			"pullapprove-review.enableEditorDecorations",
			async () => {
				const config = vscode.workspace.getConfiguration("pullapprove-review");
		...
		vscode.commands.registerCommand(
			"pullapprove-review.disableEditorDecorations",
			async () => {
				const config = vscode.workspace.getConfiguration("pullapprove-review");
		...
		vscode.commands.registerCommand(
			"pullapprove-review.markCurrentHunkReviewed",
			async () => {
				const hunkInfo = getCurrentHunkInfo(gitProvider, fileTreeProvider);
				if (hunkInfo) {
					await fileTreeProvider.setHunkReviewed(
						hunkInfo.relativePath,
						hunkInfo.hunk.hash,
						true,
					);
					await decorationProvider.refresh();
		...
		vscode.commands.registerCommand(
			"pullapprove-review.unmarkCurrentHunkReviewed",
			async () => {
				const hunkInfo = getCurrentHunkInfo(gitProvider, fileTreeProvider);
				if (hunkInfo) {
					await fileTreeProvider.setHunkReviewed(
		...
		vscode.commands.registerCommand(
			"pullapprove-review.markSelectedRangesReviewed",
			async () => {
		...
		vscode.commands.registerCommand(
			"pullapprove-review.unmarkSelectedRangesReviewed",
			async () => {
				const info = getSelectedHunksInfo(gitProvider, fileTreeProvider);
				if (info) {
					for (const hunk of info.hunks) {
		...
		vscode.commands.registerCommand(
			"pullapprove-review.markReviewed",
			async (item: FileTreeItem) => {
				...
				for (const selectedItem of items) {
					if (selectedItem.file) {
						await fileTreeProvider.setFileReviewed(
							selectedItem.file.relativePath,
							true,
						);
					} else if (selectedItem.nodePath) {
						await fileTreeProvider.setFolderReviewed(
		...
		vscode.commands.registerCommand(
			"pullapprove-review.unmarkReviewed",
			async (item: FileTreeItem) => {
				// Support multi-select: use selection if multiple items, otherwise just the clicked item
				const items =
					treeView.selection.length > 1
		...
		vscode.commands.registerCommand("pullapprove-review.markAllReviewed", async () => {
			await fileTreeProvider.markAllReviewed();
			await decorationProvider.refresh();
		}),

		vscode.commands.registerCommand(
			"pullapprove-review.clearAllReviewed",
			async () => {
		...
		vscode.commands.registerCommand(
			"pullapprove-review.revealInExplorer",
			async (item: FileTreeItem) => {
				const wsRoot = gitProvider.getWorkspaceRoot();
				if (!wsRoot || !item.file) return;
		...
		vscode.commands.registerCommand(
			...
			async (
				...
				try {
					...
					console.error("[PullApprove] Failed to open diff:", err);
					await vscode.commands.executeCommand(
						"vscode.open",
						vscode.Uri.file(filePath),
					);
		...
		vscode.commands.registerCommand(
			"pullapprove-review.addFileToNotes",
			(item: FileTreeItem) => {
				// Support multi-select: use selection if multiple items, otherwise just the clicked item
				const items =
					treeView.selection.length > 1 ? treeView.selection : [item];
				for (const selectedItem of items) {
					if (selectedItem?.file) {
		...
		vscode.commands.registerCommand("pullapprove-review.addLineReference", () => {
			const editor = vscode.window.activeTextEditor;
			if (!editor) return;
			...
			if (!selection.isEmpty) {
				...
				reviewViewProvider.addLineReference(
		...
		vscode.commands.registerCommand("pullapprove-review.addSelectionReference", () => {
			const editor = vscode.window.activeTextEditor;
			if (!editor) return;
