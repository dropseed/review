...
export class GitProvider {
  ...
  async initialize(): Promise<boolean> {
    console.log("[HumanReview] GitProvider initializing...");
    const gitExtension = vscode.extensions.getExtension<GitExtension>("vscode.git");
    if (!gitExtension) {
      console.log("[HumanReview] Git extension not found");
      return false;
    }

    console.log("[HumanReview] Git extension found, isActive:", gitExtension.isActive);
    const git = gitExtension.isActive ? gitExtension.exports : await gitExtension.activate();
    console.log("[HumanReview] Git API obtained");

    this.api = git.getAPI(1);

    // Get first repository if available
    console.log("[HumanReview] Found repositories:", this.api.repositories.length);
    if (this.api.repositories.length > 0) {
      this.repository = this.api.repositories[0];
      console.log("[HumanReview] Using repository:", this.repository.rootUri.fsPath);
  ...
  private subscribeToRepoChanges(repo: Repository): void {
    try {
      ...
      if (typeof repo.onDidChange === "function") {
        ...
        console.warn("[HumanReview] Repository.onDidChange not available, file watching disabled");
      }
    } catch (err) {
      console.error("[HumanReview] Failed to subscribe to repo changes:", err);
    }
  }
  ...
  selectRepository(repoPath: string): void {
    const repo = this.api?.repositories.find((r) => r.rootUri.fsPath === repoPath);
