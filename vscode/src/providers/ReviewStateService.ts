...
export class ReviewStateService {
	...
	private get pullapproveDir(): string | null {
		const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
		if (!workspaceFolder) return null;
		return path.join(workspaceFolder.uri.fsPath, ".pullapprove");
	}

	private get currentFile(): string | null {
		const dir = this.pullapproveDir;
		if (!dir) return null;
		return path.join(dir, "current");
	}

	/**
	 * Get the current comparison key from .pullapprove/current file.
	...
	private ensureDirectory(): void {
		...
		const pullapproveDir = path.dirname(this.stateDir);

		if (!fs.existsSync(this.stateDir)) {
			fs.mkdirSync(this.stateDir, { recursive: true });
		}

		// Create .gitignore in pullapprove dir to ignore itself
		const gitignorePath = path.join(pullapproveDir, ".gitignore");
	...
	private parseComparisonKey(comparisonKey: string): ComparisonKey {
		...
		const keyWithoutPlus = workingTree
			? comparisonKey.slice(0, -1)
			: comparisonKey;
