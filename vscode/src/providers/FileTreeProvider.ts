...
export class FileTreeProvider implements vscode.TreeDataProvider<FileTreeItem> {
	...
	private rebuildTrees(): void {
		...
		const toReviewFiles = this.files.filter((f) =>
			this.hasAnyUnreviewedHunk(f),
		);
	...
	async getChildren(element?: FileTreeItem): Promise<FileTreeItem[]> {
		...
		if (element.section) {
			const tree =
				element.section === "reviewed" ? this.reviewedTree : this.toReviewTree;
			if (!tree) return [];
		...
		if (element.nodePath && !element.file) {
			const tree =
				element.isReviewed === true ? this.reviewedTree : this.toReviewTree;
			if (!tree) return [];
	...
	private getNodeChildren(
		...
		for (const child of sorted) {
			if (child.file) {
				...
				items.push(
					FileTreeItem.createFile(child.file, isReviewedSection, child.name),
				);
			} else {
				// Add folder
				items.push(
					FileTreeItem.createFolder(child.name, child.path, isReviewedSection),
				);
			}
		}
...
export class FileTreeItem extends vscode.TreeItem {
	...
	static createSection(section: SectionType, count: number): FileTreeItem {
		...
		item.contextValue =
			section === "toReview" ? "toReviewSection" : "reviewedSection";
	...
	static createFile(
		...
		item.command = {
			command: "pullapprove-review.openDiff",
			title: "Open Diff",
